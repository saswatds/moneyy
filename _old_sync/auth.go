...
import (
	...
	"encore.app/sync/internal/encryption"
...
func init() {
	// For local development - in production this should come from Encore secrets
	if secrets.EncryptionMasterKey == "" {
		secrets.EncryptionMasterKey = "nChSLfEzS8Rl7MSHKN8EPU1Rk8Pqc9dGLowhSj9uKsQ="
	}
}

// InitiateWealthsimpleConnectionReal implements the real Wealthsimple connection flow
func initiateWealthsimpleConnectionReal(ctx context.Context, userID, username, password string) (*InitiateConnectionResponse, error) {
	// Initialize encryption service
	encService, err := encryption.NewService(secrets.EncryptionMasterKey)
	if err != nil {
...
func verifyOTPReal(ctx context.Context, credentialID, otpCode string) (*VerifyOTPResponse, error) {
	...
	rlog.Debug("received token response",
		"identity_id", tokenResp.IdentityCanonicalID,
	...
	if identityID == "" {
		...
		rlog.Info("extracted identity from JWT", "identity_id", identityID)
	}
	...
	rlog.Debug("storing identity_canonical_id",
	...
	go func() {
		...
		if err := performInitialSync(bgCtx, creds.UserID, credentialID); err != nil {
			rlog.Error("initial sync failed", "error", err, "credential_id", credentialID)
			_, _ = db.Exec(bgCtx, `
				UPDATE sync_credentials
...
func refreshAccessToken(ctx context.Context, credentialID string) error {
	// Get credential
	var creds struct {
...
func getDecryptedCredentials(ctx context.Context, userID string) (*wealthsimple.Client, error) {
	var creds struct {
		ID                    string
	...
	if creds.TokenExpiresAt.Valid && time.Now().After(creds.TokenExpiresAt.Time.Add(-5*time.Minute)) {
		...
		if err := refreshAccessToken(ctx, creds.ID); err != nil {
