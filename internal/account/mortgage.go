...
func (s *Service) GetAmortizationSchedule(ctx context.Context, accountID string) (*AmortizationScheduleResponse, error) {
	// Get mortgage details
	details, err := s.GetMortgageDetails(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) SyncMortgageBalance(ctx context.Context, accountID string) error {
	// Get mortgage details
	details, err := s.GetMortgageDetails(ctx, accountID)
	if err != nil {
	...
	if err != nil && err != sql.ErrNoRows {
		return fmt.Errorf("failed to query latest payment: %w", err)
	}
