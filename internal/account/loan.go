...
func (s *Service) GetLoanAmortizationSchedule(ctx context.Context, accountID string) (*AmortizationScheduleResponse, error) {
	// Get loan details
	details, err := s.GetLoanDetails(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) SyncLoanBalance(ctx context.Context, accountID string) error {
	// Get loan details
	details, err := s.GetLoanDetails(ctx, accountID)
	if err != nil {
